...
def parseLogs(i, core, itr, d, rapl, fname):
            ...
            if int(tmp3[13]) > START_RDTSC and int(tmp[13]) < END_RDTSC and int(tmp3[5]) > 0 and int(tmp3[6]) > 0 and int(tmp3[7]) > 0 and int(tmp3[8]) > 0:
                ...
                if joules > 0:
                    ...
                    if c7 > 0 and int(tmp3[11]) > c7:
                        ...
                        c7 = int(tmp3[11])
    f.close()
    tjoules += (JOULE_CONVERSION * (endj-startj))
    tins += total_ins
    tcyc += total_cyc
    ...
    #if core == 0 or core == 1:
    #    print("linux "+str(i)+" "+str(core)+" "+str(itr)+" "+str(d)+" "+str(rapl)+" "+str(mqps)+" "+str(tdiff)+" "+str(round((JOULE_CONVERSION * (endj-startj)),2))+" "+str(total_ins)+" "+str(total_cyc)+" "+str(total_refcyc)+" "+str(total_llcm)+" "+str(total_c3)+" "+str(total_c6)+" "+str(total_c7)+" "+str(read_5th)+" "+str(read_10th)+" "+str(read_50th)+" "+str(read_90th)+" "+str(read_95th)+" "+str(read_99th)+" "+str(START_RDTSC)+" "+str(END_RDTSC))
    #else:
    #    print("linux "+str(i)+" "+str(core)+" "+str(itr)+" "+str(d)+" "+str(rapl)+" "+str(mqps)+" "+str(tdiff)+" "+str(0.0)+" "+str(total_ins)+" "+str(total_cyc)+" "+str(total_refcyc)+" "+str(total_llcm)+" "+str(total_c3)+" "+str(total_c6)+" "+str(total_c7)+" "+str(read_5th)+" "+str(read_10th)+" "+str(read_50th)+" "+str(read_90th)+" "+str(read_95th)+" "+str(read_99th)+" "+str(START_RDTSC)+" "+str(END_RDTSC))
