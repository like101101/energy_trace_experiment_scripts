...
export RITRS=${RITRS:-"2 3 4 6 8 10 12 14 16 18 20 40"}
		...
		for nrepeat in `seq 0 1 ${NITERS}`; do		    
			...
			for rerun in `seq 0 1 1`; do
				...
				if reboot; then	    
				    runBench=0
				    echo "reboot success"
				    timeout 300 python3 -u nodejs_bench.py --os ebbrt --rapl 135 --dvfs 0x1d00 --itr 20 --nrepeat 0
