...
if len(sys.argv) != 5:
    print("clean.py itr dvfs rapl num")
    exit()
dvfs = []
itrs = []
rapls = []
...
num.append(sys.argv[4])

TIME_CONVERSION_khz = 1./(2899999*1000)
JOULE_CONVERSION = 0.00001526

#iifwh = open('linux_nodejs_all.csv', 'a+')
#fwh.write("\nsys i itr dvfs rapl lat50 lat75 lat90 lat99 Request Time Joule total_ins total_cyc total_refcyc total_llcm c3 c6 c7\n")
#sys i itr dvfs rapl lat50 lat75 lat90 lat99 requests time joule ins cyc refcyc llcm c3 c6 c7 START_RDTSC END_RDTSC
                ...
                if not path.exists(fname):
                    ...
                    for line in f:
                        ...
                        if len(tmp) > 5 and 'i' not in line.strip():
                            ...
                            if int(tmp3[13]) > START_RDTSC and int(tmp3[13]) < END_RDTSC:
                                rxd += int(tmp3[1])
                                rxb += int(tmp3[2])
                                txd += int(tmp3[3])
                                txb += int(tmp3[4])
                            ...
                            if int(tmp3[13]) > START_RDTSC and int(tmp3[13]) < END_RDTSC and int(tmp3[5]) > 0 and int(tmp3[6]) > 0 and int(tmp3[7]) > 0 and int(tmp3[8]) > 0:
                                ...
                                if c7 > 0 and int(tmp3[11]) > c7:
                                    ...
                                    #print(sumj, round(JOULE_CONVERSION * sumj, 2))
                    #print(startj,endj, (endj-startj), round((JOULE_CONVERSION *(endj-startj))))
                    #total_ins = eins - ins
                    #total_cyc = ecyc - cyc
                    #total_refcyc = erefcyc - refcyc
                    #total_llcm = ellcm - llcm
                    #total_c3 = ec3 - c3
                    #total_c6 = ec6 - c6
                    #total_c7 = ec7 - c7
                    ...
                    #print("linux "+str(i)+" "+str(itr)+" "+str(d)+" "+str(rapl)+" "+str(lat_us_50)+" "+str(lat_us_75)+" "+str(lat_us_90)+" "+str(lat_us_99)+" "+str(total_requests)+" "+str(round(tdiff, 2))+" "+str(round(JOULE_CONVERSION * sumj, 2))+" "+str(total_ins)+" "+str(total_cyc)+" "+str(total_refcyc)+" "+str(total_llcm)+" "+str(total_c3)+" "+str(total_c6)+" "+str(total_c7)+" "+str(START_RDTSC)+" "+str(END_RDTSC))
                    print("linux", i, itr, d, rapl, total_requests, "rxd=", rxd, "rxbyte=", rxb, " txd=", txd, " txbyte", txb, "receive byte/desc=", rxb/rxd, "transmit byte/desc=", txb/txd)
