...
'''
dvfs = ["0x1d00", "0x1c00", "0x1b00", "0x1a00", "0x1900", "0x1800"]
...
def parseOut(i, itr, d, rapl, q):
    ...
    f = 'mcdsilo_out.'+str(i)+'_'+itr+'_'+d+'_'+rapl+'_'+q
    #print(f)
    fout = open(f, 'r')
    for line in fout:
...
def parseRdtsc(i, itr, d, rapl, q, fname):
    global START_RDTSC
    global END_RDTSC
    global tdiff

    f = 'mcdsilo_rdtsc.'+str(i)+'_1_'+itr+'_'+d+'_'+rapl+'_'+q
    #print(f)
    frtdsc = open(f, 'r')
    START_RDTSC = 0
    END_RDTSC = 0
...
def parseLogs(i, core, itr, d, rapl, fname):
    ...
    for line in f:
        ...
        if 'rxdesc' in line.strip():
            start_check = True
            
        if len(tmp) > 5 and 'i' not in line.strip() and start_check == True:
            tmp2 = list(filter(None, line.strip().split(']')))
            tmp3 = list(filter(None, tmp2[1].strip().split(' ')))                            

            tscdiff = round(float((int(tmp3[13]) - START_RDTSC) * TIME_CONVERSION_khz), 2)            
            #if int(tmp3[13]) > START_RDTSC and int(tmp3[13]) < END_RDTSC and int(tmp3[5]) > 0 and int(tmp3[6]) > 0 and int(tmp3[7]) > 0 and int(tmp3[8]) > 0:
            if int(tmp3[13]) > START_RDTSC and tscdiff < 20.01 and int(tmp3[5]) > 0 and int(tmp3[6]) > 0 and int(tmp3[7]) > 0 and int(tmp3[8]) > 0:
                END_RDTSC = int(tmp3[13])
                joules = int(tmp3[12])
                if joules > 0:
                    if prevj == 0:
                        prevj = joules
...
print("sys i core itr dvfs rapl QPS time joule ins cyc refcyc llcm c3 c6 c7 read_5th read_10th read_50th read_90th read_95th read_99th RDTSC_START RDTSC_END REAL_QPS")
            ...
            for rapl in rapls:
                for i in range(0, iters):
                    ...
                    for core in range(0, 15):
                        fname = 'mcdsilo_dmesg.'+str(i)+'_'+str(core)+'_'+itr+'_'+d+'_'+rapl+'_'+qps
                        print(fname)
                        if not path.exists(fname):
                            print(fname, "doesn't exist?")
                        else:                    
                            parseOut(i, itr, d, rapl, qps)
                            parseRdtsc(i, itr, d, rapl, qps, fname)
                    ...
                    print("linux "+str(i)+" "+str(itr)+" "+str(d)+" "+str(rapl)+" "+str(mqps)+" "+str(round(float((END_RDTSC - START_RDTSC) * TIME_CONVERSION_khz), 2))+" "+str(round(tjoules,2))+" "+str(tins)+" "+str(tcyc)+" "+str(trefcyc)+" "+str(tllcm)+" "+str(tc3)+" "+str(tc6)+" "+str(tc7)+" "+str(read_5th)+" "+str(read_10th)+" "+str(read_50th)+" "+str(read_90th)+" "+str(read_95th)+" "+str(read_99th)+" "+str(START_RDTSC)+" "+str(END_RDTSC)+" "+str(qps))
