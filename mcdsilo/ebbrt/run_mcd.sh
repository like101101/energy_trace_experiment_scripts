	...
	for itr in $ITRS; do
		...
		for r in ${MRAPL}; do
			...
			for rerun in `seq 0 1 2`; do
				...
				echo "ebbrt_rdtsc.${nrepeat}_${ritr}_${dvfs}_${r}_${qps}"
...
function runEbbRTpart
{
   ...
   # MDVFS="0x1500" MQPS="200000" ITRS="50" MRAPL="135" NITERS="7" runEbbRT
    ...
    #50000 100000 200000
    #sleep 1    
    #MDVFS="0x1100 0xf00 0xd00" ITRS="25 50 100 150 200" MQPS="200000 400000 600000" MRAPL="135 95 55" runEbbRT
}
...
{
    #runMutilateBench --bench mcd --qps 200000 --time 30 --itr 50 --rapl 95 --dvfs 0x1d00 --nrepeat 0
    qps="200000"
    for r in "95" "75"; do
	for itr in $ITRS; do
	    for dvfs in $MDVFS; do	    
		echo "runMutilateBench --bench mcd --qps ${qps} --time 30 --itr ${itr} --rapl ${r} --dvfs ${dvfs} --nrepeat 0"
		runMutilateBench --bench mcd --qps ${qps} --time 30 --itr ${itr} --rapl ${r} --dvfs ${dvfs} --nrepeat 0
	    done
	done
    done
}
